...
public class SudokuGenerator {
    ...
    public List<SudokuCell> removeElements(List<SudokuCell> mList, int iLevel) {
        int i = 0;
        while (i < iLevel) {
            int x = rand.nextInt(80);
            if (!mList.get(x).isHide()) {
                mList.get(x).setHide(true);
                mList.get(x).setAnwser(false);
                i++;
            }
        }
        return mList;
    ...
    private boolean checkVerticalConflict(final int[][] Sudoku,
                                          final int xPos, final int yPos, final int number) {
